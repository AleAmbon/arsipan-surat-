...
$query = mysqli_query($koneksi, "SELECT * FROM surat_keluar ORDER BY created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Surat Keluar - Arsip Surat</title>
    ...
    <style>
        ...
        * {
        ...
        .btn-add:hover { transform: translateY(-1px); box-shadow: 0 4px 12px rgba(236, 72, 153, 0.3); }
...
<body>
    ...
    <div class="main-content">
        <div class="header">
            <h1 style="font-size: 1.75rem; font-weight: 800;">Daftar Surat Keluar</h1>
            <a href="tambah_surat_keluar.php" class="btn-add">
                    ...
                    <?php if(mysqli_num_rows($query) > 0): ?>
                        <?php while($row = mysqli_fetch_array($query)): ?>
                            <tr>
                                ...
                                <td style="font-weight: 600;"><?php echo $row['tujuan_surat']; ?></td>
                                ...
                                <td class="action-btns">
                                    <?php 
                                        $wa_text = "Halo, berikut rincian Surat Keluar:%0A" . 
                                                   "No. Surat: " . $row['no_surat'] . "%0A" .
                                                   "Tujuan: " . $row['tujuan_surat'] . "%0A" .
                                                   "Tanggal: " . date('d M Y', strtotime($row['tgl_surat'])) . "%0A" .
                                                   "Perihal: " . $row['perihal'];
                                        
                                        $email_subject = "Data Surat Keluar - " . $row['no_surat'];
                                    ...
                                    <?php if($_SESSION['role'] == 'admin'): ?>
                                        <a href="edit_surat_keluar.php?id=<?php echo $row['id']; ?>" class="btn-icon btn-edit" title="Edit"><i class="fas fa-pen-to-square"></i></a>
                                        <a href="hapus_surat_keluar.php?id=<?php echo $row['id']; ?>" class="btn-icon btn-delete" title="Hapus" onclick="return confirm('Apakah Anda yakin ingin menghapus surat ini?')"><i class="fas fa-trash"></i></a>
                                    <?php endif; ?>
                                 </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="5" style="text-align: center; color: var(--text-muted); padding: 3rem;">
                                <i class="fas fa-paper-plane" style="font-size: 2rem; display: block; margin-bottom: 1rem; opacity: 0.3;"></i>
